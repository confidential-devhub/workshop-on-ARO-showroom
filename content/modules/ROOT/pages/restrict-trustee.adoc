...
pass:[<span style="color: red;"><b>PERSONA:</b></span>] Application developer & Operational security expert (trustee management)
...
* `pod1` can only access the secret `fraud-dataset/dataset_key`
* `pod2` can only access the secret `p2-secret/secret`
* `pod3` can access any other secret except for `fraud-dataset` and `p2-secret`.
...
After Trustee is configured, we will see that if we try to access `fraud-dataset` with `pod2` or `pod3`, the attestation check will be succesful but the xref:configure-trustee.adoc#trustee-rap[resource access policy] will deny its access.
...
In case you didn't do it xref:curl_pod.adoc#load-key-trustee[before], download the decryption key for `pod1` and upload it into the Trustee (`fraud-dataset/dataset_key`).
...
You should see `fraud-dataset` and `p2-secret`.
...
Second, we enabled logs, and restricted exec to just try to pull `fraud-dataset`, `p2-secret` and the default `kbsres1`.
...
* `pod1` **can** access `fraud-dataset/dataset_key`
+
[source,sh,role=execute]
----
oc exec -it pods/pod1 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/fraud-dataset/dataset_key && echo ""
----
+
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/pod1 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/fraud-dataset/dataset_key && echo ""
<binary content of the key>
----
...
* `pod2` **cannot** access `fraud-dataset/dataset_key`
+
[source,sh,role=execute]
----
oc exec -it pods/pod2 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/fraud-dataset/dataset_key && echo ""
----
+
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/pod2 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/fraud-dataset/dataset_key && echo ""
rpc status: Status { code: INTERNAL, message: "[CDH] [ERROR]: Get Resource failed", details: [], special_fields: SpecialFields { unknown_fields: UnknownFields { fields: None }, cached_size: CachedSize { size: 0 } } }
----
...
* `pod3` **cannot** access `fraud-dataset/dataset_key`
+
[source,sh,role=execute]
----
oc exec -it pods/pod3 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/fraud-dataset/dataset_key && echo ""
----
+
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/pod3 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/fraud-dataset/dataset_key && echo ""
rpc status: Status { code: INTERNAL, message: "[CDH] [ERROR]: Get Resource failed", details: [], special_fields: SpecialFields { unknown_fields: UnknownFields { fields: None }, cached_size: CachedSize { size: 0 } } }
----
