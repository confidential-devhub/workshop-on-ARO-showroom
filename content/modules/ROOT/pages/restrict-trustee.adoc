...
pass:[<span style="color: red;"><b>PERSONA:</b></span>] Application developer & Operational security expert

ifeval::["{azure_resource_group}" != "insert_your_rg_here"]
NOTE: Remember to use the `devel` (xref:index.adoc#panels[main window, lower tab]) user to perform developer steps, and the `opsec` user to perform the operational security expert steps!
endif::[]

So far, for simplicity we always allowed all pods to be able to access the same Trustee resources.
...
All three pods will have the same restrictions: logs enabled, exec restricted to only `curl p1-secret/dataset_key`, `curl p2-secret/secret` and `curl attestation-status/status`.

After Trustee is configured, we will see that if we try to access `p1-secret` with `pod2` or `pod3`, the attestation check will be succesful but the xref:configure-trustee.adoc#trustee-rap[resource access policy] will deny its access.

image::resource_access.png[link=self, window=blank]
...
Note that for this example, the actual workload is not really important, therefore to keep things as simple as possible, we will use the xref:simple_pod.adoc[CoCo pod with default settings] deployment example for all three pods, and fetch the secrets manually via exec.
...
policy_data := {
  "allowed_commands": [
        ...
        "curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status",
  ]
}
...
Second, we enabled logs, and restricted exec to just try to pull `p1-secret`, `p2-secret` and the default `attestation-status`.
...
oc get configmap trusteeconfig-resource-policy \
  -n trustee-operator-system \
  -o jsonpath='{.data.resource-policy\.rego}'
----
...
oc create configmap trusteeconfig-resource-policy \
  -n trustee-operator-system \
  --from-file=resource-policy.rego=custom-resourcepolicy.rego \
...
As last step, we need to update the xref:configure-trustee.adoc#trustee-refval[reference values] to also accept the three new pcr8 values:
...
image::p1.png[link=self, window=blank]
...
----
* `pod1` **can** access `attestation-status/status`
+
[source,sh,role=execute]
----
oc exec -it pods/pod1 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
----
+
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/pod1 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
resval1
----

Same logic applies to `pod2`

image::p2.png[link=self, window=blank]
...
Pod 2 secret!
----
* `pod2` **can** access `attestation-status/status`
+
[source,sh,role=execute]
----
oc exec -it pods/pod2 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
----
+
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/pod2 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
resval1
----

The interesting part comes with `pod3`

image::p3.png[link=self, window=blank]
...
rpc status: Status { code: INTERNAL, message: "[CDH] [ERROR]: Get Resource failed", details: [], special_fields: SpecialFields { unknown_fields: UnknownFields { fields: None }, cached_size: CachedSize { size: 0 } } }
----
* `pod3` **can** access `attestation-status/status`
+
[source,sh,role=execute]
----
oc exec -it pods/pod3 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
----
+
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/pod3 -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
resval1
----
...
[source,sh,role=execute]
----
oc delete pod pod1 pod2 pod3 --wait=false
----