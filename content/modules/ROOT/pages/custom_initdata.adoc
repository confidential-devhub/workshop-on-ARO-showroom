...
What the above means is that the defined commands to `curl attestation-status/status` and `uname -r` will work, but **any** other command will fail. Even `oc exec -it pods/your_pod -- curl http://127.0.0.1:8006/cdh/resource/default/attestation-status/status2` will not work.
...
echo "$CUSTOM_INITDATA_B64" > $BASE_DIR/custom-initdata-b64.toml
...
[source,sh,role=execute]
----
# simulate Opsec -> developer handover of initdata
CUSTOM_INITDATA_B64=$(cat $BASE_DIR/custom-initdata-b64.toml)
----
...
Since this is one of the only commands allowed, let's try to `exec` to get the example Trustee secret `attestation-status/status` into the pod.

[source,sh,role=execute]
----
oc exec -it pods/initdata-fraud-detection -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
----
...
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/initdata-fraud-detection -n default -- curl -s http://127.0.0.1:8006/cdh/resource/default/attestation-status/status && echo ""
success
----
...
[source,sh,role=execute]
----
oc exec -it pods/initdata-fraud-detection -n default -- ls && echo ""
----
...
[source,sh,role=execute]
----
oc delete pods/initdata-fraud-detection -n default --wait=false
----
