...
Once that Trustee is completely set up, we can inject our custom initdata and see the pod running with a slightly relaxed but still secure policy.
...
[source,sh,role=execute]
----
oc exec -it pods/initdata-fraud-detection -- bash
----
...
[source,sh,role=execute]
----
oc exec -it pods/initdata-fraud-detection -- curl -s http://127.0.0.1:8006/cdh/resource/default/kbsres1/key1 && echo ""
----
...
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/initdata-fraud-detection -- curl -s http://127.0.0.1:8006/cdh/resource/default/kbsres1/key1 && echo ""
resval1
----
...
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/initdata-fraud-detection -- ls
error: Internal error occurred: error executing command in container: cannot enter container ba883405d94b151886307518daa04171716edcb410e8b4ff556a76ca951fbfa9, with err rpc error: code = PermissionDenied desc = "ExecProcessRequest is blocked by policy: ": unknown
----
