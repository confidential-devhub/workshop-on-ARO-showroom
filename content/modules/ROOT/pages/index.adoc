...
In this workshop, we are also showing another operator, the **Red Hat build of Trustee** (also known as **confidential compute attestation operator** and referred in this guide as `Trustee`), which can verify the trustworthiness of TEEs remotely. For more information, please refer to https://www.redhat.com/en/blog/introducing-confidential-containers-trustee-attestation-services-solution-overview-and-use-cases[this blogpost, window=blank].

We will show how to set up the Trustee and OSC operator and run a simple fraud-detection **Confidential Container** running with the peer pods solution. This effectively means that the fraud-detection container runs in a **separate**, **confidential**, **independent** virtual machine, and not in the worker node. In another example, we will also show how **attestation** and **secure key retrieval** workflow happens between a CoCo pod and Trustee.
...
* https://www.redhat.com/en/blog/learn-about-confidential-computing-attestation[Learn about confidential computing attestation]
* xref:index-theory.adoc[This workshop's theory chapter]
...
This workshop demystifies the CoCo threat model and architecture by demonstrating the roles and knowledge requirements of 3 key personas in a typical Kubernetes environment:
...
. Setup phase 1: install and configure the trusted cluster (Trustee and secrets)
  ...
  .. The *application developer* provides the application-specific secrets to the OpSec persona that then uploads them in Trustee (or in a more relaxed env, uploads them directly), and creates the necessary sealed secres (more on this later).
...
. Deployment phase: the *application developer* launches various applications
  ...
  .. If they don't, it means something has been compromised or forbidden by security policies. The developer needs the operational security responsible help to understand what is happening and how to solve the issue.
...
Looking at the right side, you will notice that a `tmux` session has already been started. In the first window on top you can see the opsec persona user on top and the devel on bottom, and in the second one there's the untrusted cluster admin. Each persona is logged in his own Openshift user with a specific RBAC:

* The `opsec` user is the only one capable of accessing the Trustee namespace

* The `uadmin` user is only capable of accessing the OSC namespace

* The `devel` admin is only capable of accessing the `fraud-detection` namespace

[#panels]
image::panels.png[link=self, window=blank]
The above figure shows where each user is logged in.

This is done to mimic a real scenario situation where different persona have different access levels.

endif::[]

// TODO: How about adding a note+link about Trubleshooting section with AI help?
