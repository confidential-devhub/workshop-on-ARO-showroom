...
* The xref:index-admin.adoc[admin guide], to learn how to install and configure the operators, which configs to set up and why we need such things. This is useful for admins (operational security persona) that need to set up the cluster environment.

* The xref:index-user.adoc[user guide], where both OSC and Trustee will be automatically installed and configured, and what is left to do is just run the confidential container. This is useful for end users (developers) that want to experiment CoCo without going into the installation steps or knowing how all internals work.
