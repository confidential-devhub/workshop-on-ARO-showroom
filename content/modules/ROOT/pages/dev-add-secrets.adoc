...
From the application developer prospective, all that is needed to do to trigger attestation is to query localhost:8006 with the path of the secret that we need, and if it is received, everything is working well. CoCo internal components will take care of connecting with Trustee (as we saw in the xref:02-configure-trustee.adoc[Trustee configuration], this is coming from the initdata and measured in the reference values).
