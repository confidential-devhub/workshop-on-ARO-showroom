...
less unsigned-confidential-devhub.yaml
----

This is very similar to xref:hello-openshift.adoc[hello openshift], but the image changes: it's `quay.io/confidential-devhub/signed/unsigned-image:latest`. This image is not signed.
...
Just like in xref:hello-openshift.adoc[hello openshift], we can inspect Trustee and see in the logs how the CoCo pod fetched the image policy and the public key assigned to `quay.io/confidential-devhub/signed`, but because the internal signature check did not pass, the pod didn't ask for any secret.
