...
The CoCo components inside the CVM verify the sealed secret signature before unsealing, so we need a P-256 EC signing keypair: the private key will be used to sign sealed secrets, and the public key is uploaded to Trustee for verification.
