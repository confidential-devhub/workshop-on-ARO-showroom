...
pass:[<span style="color: red;"><b>PERSONA:</b></span>] Application developer and Operational security expert
...
* the secret is no different from a traditional openshift secret. While it requires an additional step to be created in the application namespace, this secret is no different from a traditional one, and can be accessed as volume by the pod. Trustee also supports loading secrets from Vault, using the External secrets operator, as explained in this https://developers.redhat.com/articles/2025/07/28/integrate-red-hat-build-trustee-external-secrets-operator[blogpost]
...
. The container then starts. It detects `DECRYPTION_KEY_PATH` and expects to find the key in the sealed secret volume mount (under `/sealed/`).
...
[#sealed-signing-key]
== Generate the sealed-secret signing key
...
As we saw, a sealed secret stored in the untrusted cluster is just a "pointer" to the real secret in Trustee. Because this pointer lives in the untrusted cluster, an attacker with cluster access could replace it with a different pointer — for example, one that redirects the application to fetch a different Trustee secret, or a secret from a malicious KBS endpoint entirely.

To prevent this, sealed secrets are signed using https://datatracker.ietf.org/doc/html/rfc7515[JWS (JSON Web Signature)]. The opsec expert signs each pointer with a private key, and uploads the corresponding public key to Trustee. When the CoCo components inside the CVM receive a sealed secret, they first fetch the public key from Trustee and verify the JWS signature. If the pointer was tampered with, the signature check fails and the secret is not unsealed.

We need a P-256 EC signing keypair (ES256 algorithm, JWK format): the private key stays on the bastion to sign sealed secrets, and the public key is uploaded to Trustee for verification.

[source,sh,role=execute]
----
mkdir -p $BASE_DIR/sealed-secrets
cd $BASE_DIR/sealed-secrets

pip install cryptography

# Generate P-256 EC keypair in JWK format
python3 -c "
...
b64url = lambda n, l: base64.urlsafe_b64encode(n.to_bytes(l, 'big')).rstrip(b'=').decode()

jwk = {'kty':'EC','crv':'P-256','alg':'ES256','use':'sig','kid':'sealed-signing',
       'd': b64url(nums.private_value, 32),
       'x': b64url(nums.public_numbers.x, 32),
       'y': b64url(nums.public_numbers.y, 32)}

with open('sealed-signing-private.json','w') as f: json.dump(jwk, f, indent=2)
pub = {k:v for k,v in jwk.items() if k != 'd'}
with open('sealed-signing-public.json','w') as f: json.dump(pub, f, indent=2)
print('Generated sealed-signing-private.json and sealed-signing-public.json')
"

SIGNING_SECRET_NAME=sealed-signing-key
SIGNING_SECRET_FILE=jwk_public

oc create secret generic $SIGNING_SECRET_NAME \
    --from-file=$SIGNING_SECRET_FILE=./sealed-signing-public.json \
...
oc patch kbsconfig trusteeconfig-kbs-config \
  ...
  --type=json \
  -p="[
    {\"op\": \"add\", \"path\": \"/spec/kbsSecretResources/-\", \"value\": \"$SIGNING_SECRET_NAME\"}
  ]"
...
POINTER=$(podman run --root /podvm --security-opt label=disable \
  -v $BASE_DIR/sealed-secrets/sealed-signing-private.json:/signing-key.json:Z \
  -i quay.io/openshift_sandboxed_containers/coco-tools:0.5.1 \
  secret seal \
  --signing-kid kbs:///default/$SIGNING_SECRET_NAME/$SIGNING_SECRET_FILE \
  --signing-jwk-path /signing-key.json \
  vault --resource-uri kbs:///default/${SECRET_NAME}/${KEY_NAME} --provider kbs | grep -v "Warning")
    ...
    - name: fraud-detection
      ...
      env:
        - name: DECRYPTION_KEY_PATH
          value: /sealed/decryption/dataset_key
      securityContext:
        privileged: false
...
And the container will find the content of `fraud-dataset/dataset_key` in `/sealed/decryption/dataset_key`!

NOTE: Sealed secret volumes must be mounted under the `/sealed` directory for CDH to recognize and unseal them. See the https://confidentialcontainers.org/docs/features/sealed-secrets/[CoCo sealed secrets documentation] for details.
...
[source,sh,role=execute]
----
oc delete pods/sealed-fraud-detection -n default --wait=false
----
...
Trustee logs will also show the same story as the workload:
