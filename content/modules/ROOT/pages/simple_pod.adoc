...
* How easy it is to modify an existing pod to make it run in CoCo
...
image::blackbox.png[link=self, window=blank]
...
podvm-sample-fraud-detection-c0311387         Standard_DC2as_v5
...
This is the expected behavior based on the default xref:configure-trustee.adoc#trustee-ip[INITDATA policy], where `ExecProcessRequest` is set to `false` and `ReadStreamRequest` is set to `true`.
...
[source,sh,role=execute]
----
oc delete pods/sample-fraud-detection -n default --wait=false
----
...
[source,sh,role=execute]
----
cat $BASE_DIR/trustee/initdata.toml | grep -B 1 image_
----
  ...
  "transports": {
      "docker": {
          ...
          [
              {
                  "type": "sigstoreSigned",
                  "keyPath": "kbs:///default/redhat-signature/pub-key"
              }
          ],
          "registry.redhat.io":
          [
...
Which means that inside the CoCo component, when the image of the container is coming from `quay.io/confidential-devhub/signed`, `registry.access.redhat.com` or `registry.redhat.io`, they **must** ensure it is signed, and its signature will be checked against the corresponding Trustee key.

When instead the image is **not** coming from `quay.io/confidential-devhub/signed`, `registry.access.redhat.com` or `registry.redhat.io`, they should not run it.
...
[source,sh,role=execute]
----
oc delete pods/unsigned-confidential-devhub -n default --wait=false
----
...
We can inspect Trustee and see in the logs how the CoCo pod fetched the image policy and the public key assigned to `quay.io/confidential-devhub/signed`, but because the internal signature check did not pass, the pod didn't ask for any secret.
