...
image::initcontainer.png[link=self, window=blank]
...
Let's add the decryption key into the Trustee. Here we are in the **trusted** cluster. As the encrypted dataset was already pre-uploaded and shared with you, we need to download the original key.
...
[source,sh,role=execute]
----
oc delete pods/decryption-fraud-detection -n default --wait=false
----
