...
Note that the CoCo CVM root disk size is `20`. Refer xref:02-configure-osc.adoc#pp-cm[here (`ROOT_VOLUME_SIZE`)] to change it. Minimum has to be `6`. Remember that after the change you need to xref:02-configure-osc.adoc#pp-restart[restart the OSC daemonset].
====
...
In order to create the xref:02-configure-osc.adoc#pp-cm[peer-pods configmap], it is necessary to have Trustee xref:01-install-trustee.adoc[installed] and xref:02-configure-trustee.adoc[configured].
