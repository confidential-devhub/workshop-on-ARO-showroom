...
== OCP dependency upgrade

In order to enable CoCo and OSC `1.11` on ARO, specific components introduced in OpenShift `4.18.30` are required, as the current ARO default is `4.18.26`.

If you are following this guide in an Azure/ARO environment deployed outside the RH demo environment, you are expected to upgrade to `4.18.30+` **before** installing OSC.

Upgrading a cluster might take some time, therefore **for this workshop**, we will use a targeted update script to expedite the process and only update the required dependency.

Run the following to update the workshop environment:
