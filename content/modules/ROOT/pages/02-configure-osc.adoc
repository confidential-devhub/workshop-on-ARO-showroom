...
== Temporary hack (ARO workhop only)

Unfortunately when it comes to OCP `4.18`, CoCo ships the necessary components for OSC `1.11` only in version `4.18.30`. ARO however only ships `4.18.26`.

This means that:

* In this workshop, we have a "temporary hack" that updates the components installed by `KataConfig`. This is a hack not fit anywhere else outside from the workshop.

* For ARO clusters deployed outside this demo platform, the user is expected to **first** upgrade the cluster to a more recent version, and **then** install OSC.
