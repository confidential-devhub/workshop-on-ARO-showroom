...
export INITDATA=$(cat ~/trustee/initdata.toml | gzip | base64 -w0)
----
+
[source,sh,role=execute]
----
...
=== Verification
. Make sure that the `AZURE_IMAGE_ID` in the `ConfigMap` is populated. If it isn't, wait that the job called `osc-podvm-image-creation-<random-letters>` finishes. In this ARO setup, it should take around 10 minutes.
+
[source,sh,role=execute]
----
