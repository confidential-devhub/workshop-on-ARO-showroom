...
. Create and apply the `peer-pods-configmap.yaml` ConfigMap. Note that at this point you must have already xref:configure-trustee.adoc#trustee-initdata[configured initdata] and exported `$INITDATA`.
+
[source,sh,role=execute]
----
cat > pp-cm.yaml <<EOF
