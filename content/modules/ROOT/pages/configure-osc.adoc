...
----
curl -L https://raw.githubusercontent.com/confidential-devhub/workshop-on-ARO-showroom/refs/heads/next/helpers/configure-osc-az.sh -o configure-osc.sh
...
[source,sh,role=execute]
----
curl -L https://raw.githubusercontent.com/confidential-devhub/workshop-on-ARO-showroom/refs/heads/next/helpers/configure-osc-aro.sh -o configure-osc.sh
...
Note that the CoCo CVM root disk size is `20`. Refer xref:configure-osc.adoc#pp-cm[here (`ROOT_VOLUME_SIZE`)] to change it. Minimum has to be `6`. Remember that after the change you need to xref:configure-osc.adoc#pp-restart[restart the OSC daemonset].
====
...
To complete this section, it is mandatory to have all ARO credentials at hand defined in the xref:untrusted-cluster.adoc#credentials[introduction], because they will have to be inserted in the various resources that we are going to create.

In order to create the xref:configure-osc.adoc#pp-cm[peer-pods configmap], it is necessary to have Trustee xref:install-trustee.adoc[installed] and xref:configure-trustee.adoc[configured].
...
. Create and apply the `peer-pods-configmap.yaml` ConfigMap. Note that at this point you must have already xref:configure-trustee.adoc#trustee-initdata[configured initdata] and got `$INITDATA`.
+
[source,sh,role=execute]
----
cat > pp-cm.yaml <<EOF
