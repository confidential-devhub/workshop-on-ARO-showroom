...
In this example, the `hellosecret` secret has two entries (`key1`, `key2`), which the CoCo pods retrieve.
...
. `key1` will be retrieved via xref:dev-add-secrets#lazy-attestation[lazy attestation].
. `key2` will be retireved via a xref:dev-add-secrets#sealed-secrets[sealed secret].
...
Add `key1` as a simple string containing the text `Confidential_Secret!`, and `key2` as `key.bin`.
...
And then instruct Trustee to load that secret into its deployment, by updating the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig].
...
As said previously, let's load `key2` as sealed secret in the **untrusted** cluster.
...
oc create secret generic sealed-secret --from-literal=attstatus=$SECRET -n default
----

== Run the application

Let's now run the application in the untrusted cluster.
...
podvm-hello-openshift-c0311387                Standard_DC4es_v5
...
* 1 **podvm-hello-openshift-<random chars>**. This is where the `hello-openshift` pod is actually running! Note also how the instance tyoe under `Size` column at the right side is not the same as the other VMs. It is indeed `Standard_DC4es_v5`, as specified in the OSC xref:configure-osc.adoc#pp-cm[ConfigMap]. Looking at the https://learn.microsoft.com/en-us/azure/virtual-machines/sizes/general-purpose/dcesv5-series[DCes Azure docs, window=blank] we can also confirm that this instance type is using confidential hardware.

[#verify-security]
== Retrieve a secret from Trustee (attestation)
This `hello-openshift` test pod runs under the previously configured OSC initdata policy, therefore if you followed the xref:configure-trustee.adoc#trustee-ip[workshop initdata], logs are enabled and it will be possible to exec to retrieve `key1`.
...
Since this is one of the only commands allowed, `exec` to get the Trustee `key1` secret into the pod. This key was added in Trustee when xref:configure-trustee.adoc#trustee-key[configuring it]. If you followed the exact instructions, `key1` was configured to contain `Confidential_Secret!`.
...
[source,texinfo,subs="attributes"]
----
[azure@bastion ~]# oc exec -it pods/hello-openshift -- cat /sealed/secret-value/key2 && echo ""
