...
Specify the container `registry/image` in `$SECURITY_POLICY_IMAGE`. Here is where we specify that `$SECURITY_POLICY_IMAGE` has to be signed with a key present in `default/$SIGNATURE_SECRET_NAME/$SIGNATURE_SECRET_FILE`, and both `$RH_SECURITY_POLICY_IMAGE` and `$RH_SECURITY_POLICY_IMAGE2` have to be signed with a key present in `default/$RH_SIGNATURE_SECRET_NAME/$RH_SIGNATURE_SECRET_FILE`.
