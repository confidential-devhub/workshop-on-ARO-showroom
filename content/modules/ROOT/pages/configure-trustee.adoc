...
* Container signature policy requires that the `quay.io/confidential-devhub/signed` images are signed by a specific key. Any other image is accepted.
...
Note that `$SIGNATURE_SECRET_NAME` will be later added in the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig].
...
In Trustee `1.1.0` there is currently a bug in the default attestation policy shipped with the Trusteeconfig.

Run the following script to fix it:
...
podman run \
  ...
  -v /podvm:/workdir:Z \
  -w /workdir \
  -v /home/azure/trustee/cluster-pull-secret.json:/pull-secret.json \
  -v /home/azure/trustee/initdata.toml:/initdata.toml \
...
[source,sh,role=execute]
----
# rename configmap to replace the right trusteeconfig-generated one
sed -i 's/rvps-reference-values/trusteeconfig-rvps-reference-values/g' /podvm/rvps-reference-values.yaml
