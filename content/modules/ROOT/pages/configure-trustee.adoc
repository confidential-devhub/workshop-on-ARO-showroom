...
Because of the limitations of this ARO workshop, it is not possible to set up two clusters.
...
----
curl -L https://raw.githubusercontent.com/confidential-devhub/workshop-on-ARO-showroom/refs/heads/main/helpers/configure-trustee.sh -o configure-trustee.sh

chmod +x configure-trustee.sh

export TRUSTEE_ENV="rhdp"
...
* Container signature policy requires that the `quay.io/confidential-devhub/signed`, `registry.access.redhat.com` and `registry.redhat.io` images are signed by a specific key. Any other non-signed image is rejected.
** Refer xref:configure-trustee.adoc#trustee-cisvp[here] if you want to change it.

* `INITDATA` allows logs, but disallows generic exec.
** Refer xref:configure-trustee.adoc#trustee-ip[here] if you want to change OSC `INITDATA`.
** For the OSC operator, updating initdata means updating xref:configure-osc.adoc#pp-cm[peer-pods-cm] and then xref:configure-osc.adoc#pp-restart[restart the daemonset].
** For the Trustee operator, updating initdata means updating Trustee `PCR8` in the xref:configure-trustee.adoc#trustee-refval[reference values], otherwise attestation will fail, and then xref:configure-trustee.adoc#trustee-restart[restart the Trustee deployment].
...
* Attestation token key (for more info see xref:configure-trustee.adoc#att-token-key[here])
...
Create the cert-manager resources to generate the TLS certificates::
...
Lastly, these secrets will be given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] respectively in the `kbsHttpsKeySecretName`,`kbsHttpsCertSecretName` and `kbsAuthSecretName` fields.
...
[#trustee-route]
== Get the Trustee route

When TrusteeConfig is created in "Restricted" mode, it creates a secure route with `passthrough` TLS termination for Trustee. External ingress traffic does not get decrypted by the router but is forwarded directly to Trustee pod.

Create the route and set `TRUSTEE_HOST`, which together with `TRUSTEE_CERT` will be used later when configuring the xref:configure-trustee.adoc#trustee-ip[initdata].

[source,sh,role=execute]
----
# Get the HTTPS cert
// TODO: How about using "-o jsonpath" instead of `jq` (on multiple places)
...
----
https://kbs-route-trustee-operator-system.apps.rs01nyk5.eastus.aroapp.io
----
...
* One or more secret containing the public key used to verify the container image signatures. Those have to reside in Trustee.
...
* The xref:configure-trustee.adoc#trustee-initdata[initdata] config in the CoCo pod will have a section that points to that secret
...
* Ensures that the images under `quay.io/confidential-devhub/signed` (regardless of the image or tag) are signed, otherwise they will not be run.
* Ensures that any image coming from Red Hat `registry.access.redhat.com` or `registry.redhat.io` (regardless of the image or tag) is signed, otherwise it will not be run.
* As a global policy, deny non-signed images from any other registry. This means only the signed images under `quay.io/confidential-devhub/signed`, `registry.access.redhat.com` or `registry.redhat.io` can be run. Such policy can be also more restrictive, for example allowing a specific container under the `confidential-devhub` org (by using `quay.io/confidential-devhub/my_container`), or more relaxed to accept any unsigned image and only check specific registries signature (change `reject` to `insecureAcceptAnything`). In a production environment, it is highly encouraged to have a restrictive policy.

Let's create a secret containing the public key used to verify the signature of the above container image: because the image was built by the CoCo team, we need to download it first.
...
Let's also download the Red Hat public key used to sign all RH images:
[source,sh,role=execute]
----
# Download the key
...
If you want to sign your own container, refer to the https://developers.redhat.com/products/trusted-artifact-signer/overview[Red Hat Trusted Artifact Signer, window=blank] or other tools to sign container images. A simple guide on how to use upstream `cosign` is xref:sign-pod.adoc[here].

Note that `$SIGNATURE_SECRET_NAME` and `$RH_SIGNATURE_SECRET_NAME` will be later added in the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig].
...
Specify the container `registry/image` in `$SECURITY_POLICY_IMAGE`. Here is where we for example specify that `$SECURITY_POLICY_IMAGE` has to be signed with a key present in `default/$SIGNATURE_SECRET_NAME/$SIGNATURE_SECRET_FILE`.
...
Note that this `$POLICY_SECRET_NAME` secret will be later added in the generated xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] and referred (together with `$POLICY_SECRET_FILE`) by the CoCo pod xref:configure-trustee.adoc#trustee-initdata[initdata]
...
This step will be also useful in the xref:configure-osc.adoc#pp-cm[OSC operator setup]. However, since this policy is part of the attestation and we need the expected PCR(Platform Configuration Register see xref:configure-trustee.adoc#trustee-ap[Attestation policy]) to be added in the Trustee reference values, we will create the file now.
...
[#trustee-ip]
=== Create the default initdata policy

In this section, we will create the initdata that will be later set up as global in the xref:configure-osc.adoc#pp-cm[OSC operator configmap]. This policy will be used when the CoCo pod doesn't specify a specific one in its annotations.

In this policy, we will set the Trustee address in the internal CVM components. We will use `TRUSTEE_HOST` defined previously when xref:configure-trustee.adoc#trustee-route[configuring the Trustee].
...
For more information about the possible initdata policy please check https://github.com/kata-containers/kata-containers/blob/main/docs/how-to/how-to-use-the-kata-agent-policy.md#encode-a-policy-file[alternative Kata policy, window=blank].
...
A dummy reference value config is created by `Trusteeconfig` as `trusteeconfig-rvps-reference-values` ConfigMap, and given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] in the `kbsRvpsRefValuesConfigMapName` field.

We developed a tool that is able to automatically pull the RH artifact (podVM image), verify that it's signed by RH, and extract the reference values: it's called https://github.com/confidential-devhub/veritas[veritas].

Let's use it:

[IMPORTANT]
====
Note that if the downloaded reference values do not match with the actual image being deployed by OSC, attestation will fail.
If you are not using the latest OSC release, or are unsure which version to use, please refer xref:troubleshooting.adoc#trouble-refvals[here].
====

NOTE: The `coco-tools` container image only supports `linux/amd64`. It will not run natively on ARM-based systems.
...
podman run \
  ...
  -v $BASE_DIR/trustee/initdata.toml:/initdata.toml:Z \
...
// Github instructions
ifeval::["{azure_resource_group}" == "insert_your_rg_here"]
Define where your initdata is:
[source,sh,role=execute]
----
INITDATAFILE=$BASE_DIR/trustee/initdata.toml
----
...
podman run \
  ...
  -w /workdir \
  -v ./cluster-pull-secret.json:/pull-secret.json:Z \
  -v $INITDATAFILE:/initdata.toml:Z \
  -e REGISTRY_AUTH_FILE=/pull-secret.json \
  quay.io/openshift_sandboxed_containers/coco-tools:0.5.1 \
    veritas \
    --platform azure \
...
[source,texinfo,subs="attributes"]
----
INFO: Processing OSC latest
INFO: Image: registry.redhat.io/openshift-sandboxed-containers/osc-dm-verity-image@sha256:57ceb3ca2025b414fb5921acd77307fa5cad853fcadca8299fbd6fbb6e4e85d9
INFO: Verifying image signature...
...
$ cat /podvm/rvps-reference-values.yaml
# Generated by veritas (platform: azure, tee: snp)
apiVersion: v1
kind: ConfigMap
metadata:
...
data:
  reference_value: |
    {
      "snp_pcr03": "eyJuYW1lIjoic25wX3BjcjAzIiwiZXhwaXJhdGlvbiI6IjIwOTktMTItMzFUMDA6MDA6MDBaIiwidmFsdWUiOlsiM2Q0NThjZmU1NWNjMDNlYTFmNDQzZjE1NjJiZWVjOGRmNTFjNzVlMTRhOWZjZjlhNzIzNGExM2YxOThlNzk2OSJdfQ==",
      "snp_pcr09": "eyJuYW1lIjoic25wX3BjcjA5IiwiZXhwaXJhdGlvbiI6IjIwOTktMTItMzFUMDA6MDA6MDBaIiwidmFsdWUiOlsiZDFjYWUzZGViOGQ4NTFlZjViODRlMTJlMDYyYTAxYTg0NzdhY2NhZjlmNTZiOGY1ZmNlMzcxMjI3N2Y4MjU5YiJdfQ==",
      "snp_pcr11": "eyJuYW1lIjoic25wX3BjcjExIiwiZXhwaXJhdGlvbiI6IjIwOTktMTItMzFUMDA6MDA6MDBaIiwidmFsdWUiOlsiZTQ1MmNiNjllMDZhZWVhZjFmNzM5YjIxNDU1ODBhMzg0YTQ3MGUxMWMxZWUxNDRiYmJkYTI2Y2JiZWIyY2ZkYSJdfQ==",
      "snp_pcr12": "eyJuYW1lIjoic25wX3BjcjEyIiwiZXhwaXJhdGlvbiI6IjIwOTktMTItMzFUMDA6MDA6MDBaIiwidmFsdWUiOlsiMGM1OTIwMWViN2MyZGJkODRhMjg3MDMwNGE2MjM3M2U2NmY2YWVjOTNkNTQ5ZjRiMDcwOGFlNWE2ZjQ5ZWVkMyJdfQ==",
      "snp_pcr08": "eyJuYW1lIjoic25wX3BjcjA4IiwiZXhwaXJhdGlvbiI6IjIwOTktMTItMzFUMDA6MDA6MDBaIiwidmFsdWUiOlsiOTEwMDg2YjYzYjcwMzZjMzNhN2I0ZTllYWRmYTM0NjVkZjNlMzQ2ZmUwODMwZTk5YTc5NzU2OTkxYzBlNjgzYyJdfQ=="
    }
```

where each `snp_pcrX` represents different level of attestation in the chain snp_pcr03 (UEFI), snp_pcr08 (kernel), snp_pcr09 (OS/initrd), snp_pcr11 (UKI/boot), snp_pcr12 (kernel cmdline).

// RHDP workshop
ifeval::["{azure_resource_group}" != "insert_your_rg_here"]
The reference values configmap is generated in `/podvm/rvps-reference-values.yaml`.

Let's now update the ConfigMap.

[source,sh,role=execute]
----
oc apply -f /podvm/rvps-reference-values.yaml
----
endif::[]
...
* xref:configure-trustee.adoc#trustee-tdx[tdxConfigSpec:kbsTdxConfigMapName]
* xref:configure-trustee.adoc#trustee-jwk[kbsAttestationKeySecretName and kbsAttestationCertSecretName]
* xref:configure-trustee.adoc#trustee-rap[kbsResourcePolicyConfigMapName]
* xref:configure-trustee.adoc#trustee-cm[kbsConfigMapName]
* xref:configure-trustee.adoc#trusteeconfig[kbsHttpsKeySecretName, kbsHttpsCertSecretName and kbsAuthSecretName]
* xref:configure-trustee.adoc#trustee-refval[kbsRvpsRefValuesConfigMapName]
* xref:configure-trustee.adoc#trustee-ap[kbsAttestationPolicyConfigMapName]
...
The attestation token key and certificate are loaded as secret, respectively `trusteeconfig-attestation-key-secret` and `trusteeconfig-attestation-cert-secret`, and are given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] respectively in the `kbsAttestationKeySecretName` and `kbsAttestationCertSecretName` field.
...
We will then instruct the Trustee operator where to find these keys and certs (they are secrets in OCP) via the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig].

This configmap is loaded in Trustee namespace as `trusteeconfig-kbs-config`, and given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] in the `kbsConfigMapName` field.
...
The attestation policy is created by `Trusteeconfig` as `trusteeconfig-attestation-policy` ConfigMap, and given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] in the `kbsAttestationPolicyConfigMapName` field.
...
The TDX config is created by `Trusteeconfig` as `trusteeconfig-tdx-config` ConfigMap, and given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] in the `tdxConfigSpec:kbsTdxConfigMapName` field.
...
The resource access policy is created by `Trusteeconfig` as `trusteeconfig-resource-policy` ConfigMap, and given to the xref:configure-trustee.adoc#trustee-kbsconfig[KbsConfig] in the `kbsResourcePolicyConfigMapName` field.
