...
# add the cacert into the cococtl config
sed -i -E "s@trustee_ca_cert = (''|\"\")@trustee_ca_cert = \"$CACERT_PATH\"@" ~/.kube/coco-config.toml
----
...
cococtl apply \
  ...
  --init-container \
  --init-container-img quay.io/confidential-devhub/signed/fedora:44 \
  --init-container-cmd "curl http://localhost:8006/cdh/resource/default/kbsres1/key1"
----
...
* `--init-container-img`: the image to use in the initcontainer. In this example we change it since we have a tight signature image policy.
* `--init-container-cmd`: the secret the initcontainer has to fetch. In this example, we use a default secret shipped with Trustee operator.
...
[source,sh,role=execute]
----
oc delete -f app-coco.yaml -f app-sidecar-service.yaml -f app-sealed-secrets.yaml
----
