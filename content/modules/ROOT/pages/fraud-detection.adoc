...
----
AZURE_SAS_SECRET_NAME=fraud-azure-sas
AZ_SECRET=$(podman run --root /podvm --security-opt label=disable -it quay.io/confidential-devhub/coco-tools:0.3.0 /tools/secret seal vault --resource-uri kbs:///default/${AZURE_SAS_SECRET_NAME}/azure-sas --provider kbs | grep -v "Warning")

FD_SECRET_NAME=fraud-dataset
KEY_SECRET=$(podman run --root /podvm --security-opt label=disable -it quay.io/confidential-devhub/coco-tools:0.3.0 /tools/secret seal vault --resource-uri kbs:///default/${FD_SECRET_NAME}/dataset_key --provider kbs | grep -v "Warning")
