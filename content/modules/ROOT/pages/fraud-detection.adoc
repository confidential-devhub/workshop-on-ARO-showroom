...
Before creating sealed secrets, ensure the signing key has been xref:sealed_pod.adoc#sealed-signing-key[generated and uploaded to Trustee].
...
AZURE_SAS_SECRET_NAME=fraud-azure-sas
AZ_SECRET=$(podman run --root /podvm --security-opt label=disable \
  ...
  vault --resource-uri kbs:///default/${AZURE_SAS_SECRET_NAME}/azure-sas --provider kbs | grep -v "Warning")

FD_SECRET_NAME=fraud-dataset
KEY_SECRET=$(podman run --root /podvm --security-opt label=disable \
  -v $BASE_DIR/sealed-secrets/sealed-signing-private.json:/signing-key.json:Z \
  -i quay.io/openshift_sandboxed_containers/coco-tools:0.5.1 \
  secret seal \
  --signing-kid kbs:///default/$SIGNING_SECRET_NAME/$SIGNING_SECRET_FILE \
  --signing-jwk-path /signing-key.json \
  vault --resource-uri kbs:///default/${FD_SECRET_NAME}/dataset_key --provider kbs | grep -v "Warning")
...
// TODO: Add this to check (run as opsec)
//[opsec cococtl]$ oc get -n openshift-sandboxed-containers-operator cm peer-pods-cm -o jsonpath='{.data.ROOT_VOLUME_SIZE}'
