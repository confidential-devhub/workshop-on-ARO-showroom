...
In order to use the admin user, click on the `[1:admin]` tmux window as illustrated in the xref:index.adoc#panels[picture].
endif::[]
...
ifeval::["{azure_resource_group}" != "insert_your_rg_here"]
Go into the terminal on the right and run the following command to log into Azure with the right service principal:
