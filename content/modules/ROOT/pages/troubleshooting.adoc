...
If you are reading this guide rendered in the **RHDP demo workshop**, then all the instructions should work if run in the provided interface.

If you are testing on a standard **ARO deployment** outside RHDP, remember:

. You need to upgrade the cluster to a newer version, at least `4.18.30`.
. The `ARO_VNET_NAME` command xref:02-configure-osc.adoc#pp-cm[here] might not work, if your vnet does not contain `aro` in the name. As the user is free to give the ARO vnet any name, keep in mind that could not match.
. The xref:02-configure-trustee.adoc#refval-fetch[reference values script] has some steps very specific to the RHDP environment.
.. No need to use `sudo`
.. You can create `./podvm`, no need of `/podvm`.
.. You probably don't need `chown azure:azure /podvm`
.. Delete all the `--root /podvm` references later, no need.

If you are testing on an **Azure self managed cluster**, remember:

. For SNO, you won't get `kata-oc` label into the worker node (since it's a SNO).
. Check the commented commands in tje xref:02-configure-osc.adoc#pp-cm[peer-pod-cm script]. You need to uncomment `AZURE_RESOURCE_GROUP` and replace `contains(Name, 'aro')` with `contains(Name, '')`.
. The xref:02-configure-trustee.adoc#refval-fetch[reference values script] has some steps very specific to the RHDP environment.
.. No need to use `sudo`
.. You can create `./podvm`, no need of `/podvm`.
.. You probably don't need `chown azure:azure /podvm`
...
. Created and configured the xref:02-configure-osc.adoc#pp-cm[Azure public IP address and NAT Gateway].
+
Remember to run `export AZURE_RESOURCE_GROUP={azure_resource_group}` first.
+
[source,sh,role=execute]
----
