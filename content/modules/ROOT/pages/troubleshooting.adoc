...
If you use https://cursor.com[Cursor, window=blank], you can install an optional Agent Skill that walks through systematic CoCo checks on your cluster (Trustee and OSC operators, PCR8 consistency, attestation policies, sealed secrets, and failing pods).

The skill and a companion diagnostic script live in the https://github.com/confidential-devhub/workshop-skill[workshop-skill repository, window=blank].
...
If the CVM doesn't have enough disk space (set with `ROOT_VOLUME_SIZE` in xref:configure-osc.adoc#pp-cm[peer-pods-cm]), image pulling might fail because the CoCo components need to pull the container image into the CVM and due to space fail to do so.

Usually the error is visible in the pod events:
[source,texinfo,subs="attributes"]
----
Error: CreateContainer failed: rpc status: Status { code: INTERNAL, message: "[CDH] [ERROR]: Image Pull error: Failed to pull image <YOUR_IMAGE_HERE> from all mirror/mapping locations or original location: image: <YOUR_IMAGE_HERE>, error: Errors happened when pulling image: Failed to decode layer data stream: Failed to unpack layer"
----

In order to solve this issue, try to increase `ROOT_VOLUME_SIZE` and xref:configure-osc.adoc#pp-restart[restart the OSC deployment].
...
If you update `AZURE_INSTANCE_SIZE` in xref:configure-osc.adoc#pp-cm[peer-pods-cm], make sure you xref:configure-osc.adoc#pp-restart[restart the OSC deployment].
...
Upon inspection of the OSC operator `osc-caa-ds` daemonset, you should see it is stuck printing the following logs:

[source,sh,role=execute]
----
oc logs ds/osc-caa-ds -n openshift-sandboxed-containers-operator
----
...
. Created and configured the xref:configure-osc.adoc#gateway[Azure public IP address and NAT Gateway].


=== INITDATA wrongly set

Ispect the INITDATA. Inspect the INITDATA field in the  xref:configure-osc.adoc#pp-cm[peer-pods-cm] configmap in OSC namespace, or alternatively if you are providing manually in as xref:deployment-settings.adoc#initdata-ann[annotation].
...
* Make sure the formatting and syntax is correct. Refer to xref:configure-trustee.adoc#trustee-ip[this initdata] for an example with correct syntax.

If you update the INITDATA, remember to update the xref:configure-trustee.adoc#pcr8[PCR8] in the xref:configure-trustee.adoc#trustee-refval[reference values] and xref:configure-trustee.adoc#trustee-restart[restart the Trustee deployment]! Also if INITDATA is added/updated in xref:configure-osc.adoc#pp-cm[peer-pods-cm], remember to xref:configure-osc.adoc#pp-restart[restart the OSC deployment]!
...
For example, the policy used xref:configure-trustee.adoc#trustee-cisvp[in this example] is pretty permissive, as it has `"default": [ { "type": "insecureAcceptAnything" } ]`. However, if you change the type to `reject`, then you drastically reduce the amounts of CoCo pod images you can run.

If you update the signature policy, remember to xref:configure-trustee.adoc#trustee-restart[restart the Trustee deployment]!
...
If not, then it's a routing issue. If you are using passthrough, are you using the right xref:configure-trustee.adoc#trustee-keys[https certificates]?
...
If you update the https certificates, remember to xref:configure-trustee.adoc#trustee-restart[restart the Trustee deployment]!
...
* The xref:configure-trustee.adoc#trustee-ap[attestation policy] is formatted correctly. Sometimes wrong formatting or syntax errors prevent it from being loaded.
* Same applies to the xref:configure-trustee.adoc#trustee-refval[reference values].
** If you changed the podvm image to be used, the reference values also change!
** If you changed the INITDATA, the reference values (xref:configure-trustee.adoc#pcr8[PCR8]) also change!
* Lastly check also the xref:configure-trustee.adoc#trustee-rap[resource access policy].

After ANY change to any config/secret of the Trustee operator, remember to xref:configure-trustee.adoc#trustee-restart[restart the deployment]!

[#trouble-refvals]
=== Wrong reference values

In this workshop, the xref:configure-trustee.adoc#trustee-refval[reference values] are taken from the `latest` OSC available image, as we assume the operator is freshly installed. However, if OSC is not the latest version, or has not been updated, it might happen that `latest` won't match with the OSC deployed image.
...
[NOTE]
====
Why looking the tag into the `osc-dm-verity-image` repo if we have the operator installed locally?
...
Once you found the right tag, add `--image-tag 1.11.1` to the veritas command and apply again the reference values configmap.


=== Sealed secret contains gibberish

If you mounted a sealed secret, but when reading it the application reads something like `secret=sealed.eyJhbGci...` it simply means that the attestation failed. What you are reading is the "pointer" original value inside the secret (a signed JWS token), and since attestation failed it was not replaced by the actual secret content. Check the connectivity with Trustee and its logs.
...
If your pod is running, and service/route are working, but you cannot see logs nor exec, remember that this is controlled by INITDATA. If no INITDATA is provided in the CoCo pod via  xref:deployment-settings.adoc#initdata-ann[annotation], the global default one in xref:configure-osc.adoc#pp-cm[peer-pods-cm] will be used. If no global is set, default permissive settings will be used (no signature image verification, no Trustee connection, exec and logs allowed).

Check the `policy.rego` section.

* If `default ExecProcessRequest := false`, exec will be forbidden.
** It is possible to allow only specific exec commands. Refer to the xref:configure-trustee.adoc#trustee-ip[initdata example].
...
== CoCo pod stuck on deleting

If the pod is stuck after being deleted, it could be because the underlying VM is failing to be deleted.
A simple way to delete everything is via this script. Note: this deletes all the running VMs!

[source,sh,role="execute",subs=attributes+]
----
curl -L https://raw.githubusercontent.com/confidential-devhub/workshop-on-ARO-showroom/refs/heads/main/helpers/delete-vms.sh -o delete-vms.sh
